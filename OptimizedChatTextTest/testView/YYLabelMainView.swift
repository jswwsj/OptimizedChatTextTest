// YYLabelMainView.swift
import UIKit

class YYLabelMainView: UIView {
    
    // MARK: - UI Components
    private var scrollView: UIScrollView!
    private var contentView: UIView!
    private var textLabel: YYLabel! // 替换为 YYLabel
    ...
    private func setupUI() {
        ...
        textLabel.attributedText = coreTextStr
    ...
    private func setupTextLabel() {
        textLabel = YYLabel() // 使用 YYLabel
        textLabel.translatesAutoresizingMaskIntoConstraints = false
        textLabel.backgroundColor = .white
        textLabel.numberOfLines = 0 // 多行显示
        textLabel.lineBreakMode = .byWordWrapping
        // YYLabel 特有属性配置
        textLabel.displaysAsynchronously = false // 异步绘制提升性能
        textLabel.ignoreCommonProperties = false
        textLabel.fadeOnAsynchronouslyDisplay = false
//        textLabel.clearContentsBeforeAsynchronouslyDisplay = false
        textLabel.fadeOnHighlight = true
    ...
    private func appendText() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            // 设置YYLabel的富文本
            self.textLabel.attributedText = self.coreTextStr
            
            // YYLabel 自动计算尺寸，这里仍保留尺寸计算逻辑
            let textSize = self.textLabel.sizeThatFits(CGSize(width: self.textLabel.frame.width, height: .greatestFiniteMagnitude))
            
            // 更新 ScrollView 的 contentSize
            self.updateScrollViewContentSize(textSize)
            ...
            if self.updateCount > 0 && self.updateCount % 500 == 0 {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                    ...
                    let newTextSize = self.textLabel.sizeThatFits(CGSize(width: self.textLabel.frame.width, height: .greatestFiniteMagnitude))
                    self.updateScrollViewContentSize(newTextSize)
    ...
    @objc private func clearText() {
        updateCount = 0
        coreTextStr = NSMutableAttributedString()
        ...
        let textSize = textLabel.sizeThatFits(CGSize(width: textLabel.frame.width, height: .greatestFiniteMagnitude))
        updateScrollViewContentSize(textSize)
...
extension YYLabelMainView: UIScrollViewDelegate {
